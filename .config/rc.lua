...
local function run_once(cmd_arr)
    for cmd, cmd in ipairs(cmd_arr) do
        awful.spawn.with_shell(string.format("pgrep -u $USER -fx '%s' > /dev/null || (%s)", cmd, cmd))
    end
end

run_once({ "urxvtd", "unclutter -root" }) -- entries must be separated by commas
run_once({ "wireless client", "/usr/bin/python -O /usr/share/wicd/gtk/wicd-client.py -t" })
...
--              {description = "view  previous nonempty", group = "tag"}),
    ...
    awful.key({ modkey, "Control"   }, "j", function () awful.screen.focus_bydirection("up")    end,
              {description = "focus up", group = "screen"}),
    awful.key({ modkey, "Control"   }, "k", function () awful.screen.focus_bydirection("down")  end,
              {description = "focus down", group = "screen"}),
    ...
    awful.key({ modkey,           }, "space", function () os.execute(launcher)               end,
...
awful.rules.rules = {
    ...
    { rule = { class = "Emacs" },
      properties = {rise = true,
		    keys = clientkeys,
		    buttons = clientbuttons,
		    screen = awful.screen.preferred,
		    placement = awful.placement.no_offscreen},
      callback = function (c) awful.layout.set(lain.layout.centerwork) end
    },
}
-- }}}
